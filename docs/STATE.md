...
4. Each player hand is played left to right. Twenty-one auto-stands. Bust ends the hand. Optional late surrender is legal only on the original two-card hand.
5. If any hand is live, the dealer plays to S17 by default or H17 when configured. Table-rule changes lock in when the next deal begins.
